    ...
    <div id="right-panel" class="right-panel">
		...
		<div class="animated fadeIn">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-header">Complaint Allocation</div>
                            <div class="card-body card-block">
                                <form action="{{URL('/admin/ComplaintAllocation')}}" method="POST">
                                    @csrf
                                    <div class="form-group">
                                        <select name="complaints" id="complaints" class="form-control">
                                                ...
                                                <option value="{{$data->id}}">
                                                    {{$data->cust_name}}-{{$data->cust_address}}-{{$data->cust_mobile}}-{{$data->brand_name}}
                                                </option>
                                                @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                            <select name="engineer" id="engineer" class="form-control">
                                                    ...
                                                    <option value="{{$data->id}}">{{$data->e_name}}-{{$data->e_contact}}</option>
                                                    @endforeach
                                            </select>
                                        </div>
                                    <div class="form-actions form-group"><input type="submit" name="submit" value="Allot Engineer" class="btn btn-success btn-sm"></div>
                                </form>
                            </div>
                        </div>
                    </div>
